...
export default function StoriesBar({onViewStory}: StoriesBarProps) {
  ...
  return (
    <FlatList horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.container} data={[{_addStory: true}, ...userStories] as any[]} keyExtractor={(item, index) => item._addStory ? 'add' : item.user._id || index} renderItem={({item}) => {
        if (item._addStory) {
            return (
        ...
        return (
            <TouchableOpacity style={styles.storyItem} onPress={() => onViewStory(us)}>
                <View style={styles.storyRing}>
                    <Avatar name={us.user.name} size={52} src={us.user.avatar} />
                </View>
